...
table(test_Imp$test.leuk.type, DT_pred)
# BT_pred
# FALSE TRUE
# FALSE    18    2
# TRUE      1   13
...
##Bagging
BG=Bagging(as.factor(leuk.type)~., data = train_Imp, control = Weka_control(W = "DecisionStump"))
BG_pred=predict(BG, newdata = test_Imp)
table(test_Imp$test.leuk.type, BG_pred)
# BG_pred
# FALSE TRUE
# FALSE    20    0
# TRUE      5    9
##Boosting
BT=AdaBoostM1(as.factor(leuk.type) ~ ., data = train_Imp,
                 control = Weka_control(W = "DecisionStump"))
BT_pred=predict(BT, newdata = test_Imp)
table(test_Imp$test.leuk.type, BT_pred)
#BT_pred
# FALSE TRUE
# FALSE    18    2
# TRUE      1   13